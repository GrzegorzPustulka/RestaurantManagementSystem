...
async def create_menu(menu_in: MenuCreate, db: DB, _: User):
...
async def update_menu(menu_id: str, menu_in: MenuUpdate, db: DB, _: User):
...
async def delete_menu(menu_id: str, db: DB, _: User):
